...
paused = False  # Pause state for panning and playback
...
def pause():
    """Pause panning and sound playback."""
    global paused
    paused = True
    display_queue.put(('pause', None))
    return jsonify({"status": "success", "message": "Paused"})

@app.route('/resume', methods=['GET'])
def resume():
    """Resume panning and sound playback."""
    global paused
    paused = False
    display_queue.put(('resume', None))
...
def main():
    global panning_x, panning_y, viewport_x, viewport_y, current_image_index, pan_direction_x, pan_direction_y, pan_speed, paused
    ...
    while running:
        ...
        try:
            ...
            if command == 'image':
                ...
                pygame.mixer.pause()
                print("Paused panning and playback")
            elif command == 'resume':
                pygame.mixer.unpause()
                print("Resumed panning and playback")
            elif command == 'stop':
        ...
        # Update panning
        update_needed = False
        if panning_x and image_files and current_surface and not paused:
            viewport_x += pan_direction_x * pan_speed * delta_time
            update_needed = True
        if panning_y and image_files and current_surface and not paused:
